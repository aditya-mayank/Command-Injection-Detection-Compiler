...
void handle_ping_request(const char *target_ip) {
    ...
    sys_log(LOG_DEBUG, "Running command...");
    {
        ...
        for (int _k = 0; allowed[_k]; _k++)
            if (strstr(cmd_buffer, allowed[_k]) != NULL) { valid = true; break; }
        if (!valid) { fputs("Invalid cmd_buffer\n", stderr); exit(1); }
    }
    const char* sanitize_input(const char* s);
    #define cmd_buffer sanitize_input(cmd_buffer)
...
// Explicit prototypes to avoid fatal missing OS header errors on various platforms
      ...
      #define EXEC execvp
    #endif
    char *args[] = { "ls", "-la", script_call, NULL };
    #if defined(_WIN32) || defined(_MSC_VER)
